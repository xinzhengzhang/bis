...
export async function launchProcess(udid: string, bundleID: string, preferdLogPath: string): Promise<Number|undefined> {
  const randm_value = crypto.randomUUID();
  const outputFile = `/tmp/app_path_${randm_value}.json`;
  const logFile = `/tmp/app_path_log_${randm_value}.json`;
  ...
  const processLogStream = createWriteStream(preferdLogPath);
