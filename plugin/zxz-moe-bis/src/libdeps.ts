...
async function showChooseLibsInputer(opt: {
    title: string;
    placeHolder: string;
    items: string[];
}) {
    ...
    let choose = await vscode.window.showQuickPick(opt.items, quickPickOptions);
    return choose;
}
export default class LibDepsService extends Service {
    ...
    async resolveLibDeps(ws: string) {
        var items = await vscode.commands.executeCommand<string[]>(
            "zxz-moe-bis.libs"
        );
        let lib1 = await showChooseLibsInputer({
            title: "First Lib",
            items,
            placeHolder: "First Lib",
        });
        if (!lib1) {
            return;
        }
        let lib2 = await showChooseLibsInputer({
            title: "Second Lib",
            items,
            placeHolder: "Second Lib",
        });
        if (!lib2) {
            return;
        }
    ...
    async computeLibDepsc(cwd: string, lib1: string, lib2: string) {
        try {
            let { stdout: bazelOutput } = await exec(
                `${configuration.bazelExecutablePath} info output_path`,
                { cwd }
            );
            if (!bazelOutput) {
                return;
            ...
            webviewPanel.webview.html = `<img src="data:image/svg+xml;base64, ${Buffer.from(
